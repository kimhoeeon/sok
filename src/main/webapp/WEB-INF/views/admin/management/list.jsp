...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <a href="/admin/management/form" class="btn btn-neon px-4"><i class="bi bi-pencil-square"></i> 등록</a>
</div>

<div class="premium-dark-card p-4">
    <form action="/admin/management/list" method="get" class="d-flex justify-content-end mb-4">
        <div class="input-group w-50 shadow-sm">
            <select name="category" class="form-select dark-search-bar" style="max-width: 150px;">
                <option value="">전체 분류</option>
                <option value="예산/결산" ${searchCategory eq '예산/결산' ? 'selected' : ''}>예산/결산</option>
                <option value="사업계획" ${searchCategory eq '사업계획' ? 'selected' : ''}>사업계획</option>
                <option value="규정/기타" ${searchCategory eq '규정/기타' ? 'selected' : ''}>규정/기타</option>
            </select>
            <input type="text" name="title" class="form-control dark-search-bar" placeholder="제목 검색" value="${searchTitle}">
                        ...
                        <c:forEach var="item" items="${list}">
                            <tr>
                                ...
                                <td class="text-start border-secondary">
                                    <a href="/admin/management/form?brdSeq=${item.brdSeq}" class="text-white text-decoration-none fw-bold">${item.title}</a>
                                </td>
                                ...
                                <td class="border-secondary">
                                    <a href="/admin/management/form?brdSeq=${item.brdSeq}" class="btn btn-sm btn-outline-light me-1">수정</a>
                                    <form action="/admin/management/delete" method="post" style="display:inline;" onsubmit="return confirm('삭제하시겠습니까?');">
