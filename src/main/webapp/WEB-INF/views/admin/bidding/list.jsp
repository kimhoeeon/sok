...
<div class="premium-dark-card p-4">
    <form action="/admin/bidding/list" method="get" class="d-flex justify-content-end mb-4">
        <div class="input-group w-50 shadow-sm">
            <select name="category" class="form-select dark-search-bar" style="max-width: 150px;">
                <option value="">전체 상태</option>
                <option value="진행중" ${searchCategory eq '진행중' ? 'selected' : ''}>진행중</option>
                <option value="마감" ${searchCategory eq '마감' ? 'selected' : ''}>마감</option>
            </select>
            <input type="text" name="title" class="form-control dark-search-bar" placeholder="공고명 검색" value="${searchTitle}">
                        ...
                        <c:forEach var="item" items="${list}">
                            <tr>
                                ...
                                <td class="text-start border-secondary">
                                    <a href="/admin/bidding/form?brdSeq=${item.brdSeq}" class="text-white text-decoration-none fw-bold">${item.title}</a>
                                </td>
                                <td class="border-secondary">${item.viewCnt}</td>
                                <td class="border-secondary"><fmt:formatDate value="${item.regDt}" pattern="yyyy-MM-dd" /></td>
                                <td class="border-secondary">
                                    <a href="/admin/bidding/form?brdSeq=${item.brdSeq}" class="btn btn-sm btn-outline-light me-1">수정</a>
