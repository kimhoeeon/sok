...
<div class="premium-dark-card p-4">
    <form action="/admin/report/list" method="get" class="d-flex justify-content-end mb-4">
        <div class="input-group w-50 shadow-sm">
            <select name="category" class="form-select dark-search-bar" style="max-width: 150px;">
                <option value="">전체 분류</option>
                <option value="연차보고서" ${searchCategory eq '연차보고서' ? 'selected' : ''}>연차보고서</option>
                <option value="사업보고서" ${searchCategory eq '사업보고서' ? 'selected' : ''}>사업보고서</option>
                <option value="기타자료" ${searchCategory eq '기타자료' ? 'selected' : ''}>기타자료</option>
            </select>
            <input type="text" name="title" class="form-control dark-search-bar" placeholder="보고서 제목 검색" value="${searchTitle}">
                        ...
                        <c:forEach var="item" items="${list}">
                            <tr>
                                ...
                                <td class="text-start border-secondary">
                                    <a href="/admin/report/form?brdSeq=${item.brdSeq}" class="text-white text-decoration-none fw-bold">${item.title}</a>
                                </td>
                                ...
                                <td class="border-secondary">
                                    <a href="/admin/report/form?brdSeq=${item.brdSeq}" class="btn btn-sm btn-outline-light me-1">수정</a>
