...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h3 class="fw-bold text-dark">증명서 신청 상세 확인</h3>
    <a href="/mng/certificate/list?pageNum=${params.pageNum}&amount=${params.amount}&searchType=${params.searchType}&searchStatus=${params.searchStatus}&searchKeyword=${params.searchKeyword}" class="btn btn-outline-light"><i class="bi bi-list"></i> 목록으로</a>
</div>

<div class="row g-4">
    <div class="col-xl-8">
        <div class="premium-card p-5 h-100">
            <h5 class="fw-bold text-dark border-bottom border-secondary pb-3 mb-4"><i class="bi bi-person-vcard me-2 text-info"></i> 신청자 및 증명서 정보</h5>

            <div class="row mb-3">
                <div class="col-md-3 text-muted">증명서 종류</div>
                <div class="col-md-9 text-dark fw-bold"><span class="badge bg-secondary">${certificate.certType}</span></div>
            </div>
            <div class="row mb-3">
                <div class="col-md-3 text-muted">신청자명</div>
                <div class="col-md-9 text-dark">${certificate.applyNm}</div>
            </div>
            <div class="row mb-3">
                <div class="col-md-3 text-muted">연락처</div>
                <div class="col-md-9 text-dark">${certificate.phone}</div>
            </div>
            <div class="row mb-3">
                <div class="col-md-3 text-muted">이메일</div>
                <div class="col-md-9 text-dark">${certificate.email}</div>
            </div>
            <div class="row mb-3">
            ...
            <h5 class="fw-bold text-dark border-bottom border-secondary pb-3 mb-4"><i class="bi bi-journal-text me-2 text-primary"></i> 비고 (특이사항)</h5>
            <div class="p-4 rounded text-dark" style="background: rgba(255,255,255,0.03); border: 1px solid #474761; min-height: 100px; white-space: pre-wrap;">${not empty certificate.remark ? certificate.remark : '특이사항 없음'}</div>
        </div>
    </div>

    <div class="col-xl-4">
        <div class="premium-card p-4 h-100 glassmorphism-box border-0">
            <h5 class="fw-bold text-dark mb-4"><i class="bi bi-gear-fill me-2 text-primary"></i> 진행 상태 관리</h5>
            ...
            <form action="/mng/certificate/updateStatus" method="post">
                ...
                <select name="issueStatus" id="issueStatusSelect" class="form-select search-bar mb-3 border-primary" onchange="toggleRejectReason()">
                ...
                <div id="rejectReasonDiv" style="display: ${certificate.issueStatus eq 'REJECT' ? 'block' : 'none'};">
                    ...
                    <textarea name="rejectRsn" id="rejectRsn" class="form-control search-bar border-danger mb-3" rows="3" placeholder="반려/거절 사유를 입력해주세요.">${certificate.rejectRsn}</textarea>
                </div>
