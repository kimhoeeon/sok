...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h3 class="fw-bold text-dark">회원/후원자 상세 정보</h3>
    <a href="/mng/sponsor/member/list?pageNum=${params.pageNum}&amount=${params.amount}&withdrawYn=${params.withdrawYn}&mbrType=${params.mbrType}&isDonor=${params.isDonor}&searchKeyword=${params.searchKeyword}" class="btn btn-outline-light"><i class="bi bi-list"></i> 목록으로</a>
</div>
...
<div class="row g-4">
        ...
        <div class="premium-card p-4 h-100">
            ...
            <form action="/mng/sponsor/member/update" method="post">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control search-bar" value="${member.mbrType eq 'CORP' ? '기업/단체' : '개인'} / ${not empty member.bizNo ? member.bizNo : '-'}" disabled>
                </div>
                <div class="mb-3">
                    <label class="text-muted mb-1">연락처</label>
                    <input type="text" name="phone" class="form-control search-bar border-primary" value="${member.phone}">
                </div>
                <div class="mb-3">
                    <label class="text-muted mb-1">이메일</label>
                    <input type="email" name="email" class="form-control search-bar border-primary" value="${member.email}">
                </div>
                ...
                <c:if test="${member.mbrType eq 'CORP'}">
                    <div class="row mb-3">
                        <div class="col-6">
                            ...
                            <input type="text" name="managerNm" class="form-control search-bar border-primary" value="${member.managerNm}">
                        </div>
                        <div class="col-6">
                            <label class="text-muted mb-1">담당자 직함</label>
                            <input type="text" name="managerPos" class="form-control search-bar border-primary" value="${member.managerPos}">
                        </div>
                    </div>
                </c:if>
    ...
    <div class="col-xl-7">
        <div class="premium-card p-4 h-100">
            <h5 class="fw-bold text-dark border-bottom border-secondary pb-3 mb-4"><i class="bi bi-piggy-bank-fill me-2 text-danger"></i> 해당 회원의 기부 결제 내역</h5>
            ...
            <div class="table-responsive" style="max-height: 400px; overflow-y: auto;">
                <table class="table table-hover align-middle text-center" style="font-size: 13px;">
                    <thead style="position: sticky; top: 0; background: #1e1e2d; z-index: 1;">
                        <tr>
                            <th class="text-dark border-bottom border-secondary">주문번호</th>
                            <th class="text-dark border-bottom border-secondary">기부유형</th>
                            <th class="text-dark border-bottom border-secondary">결제금액</th>
                            <th class="text-dark border-bottom border-secondary">결제상태</th>
                            <th class="text-dark border-bottom border-secondary">결제일시</th>
                        </tr>
                    </thead>
                        ...
                        <c:choose>
                            ...
                            <c:otherwise>
                                <c:forEach var="d" items="${member.donationList}">
                                    <tr>
                                        <td><a href="/mng/sponsor/donate/detail?paySeq=${d.paySeq}" class="text-info text-decoration-none">${d.orderId}</a></td>
                                        <td>
                                            <c:choose>
